...
meta_current_price = meta_data.history(end=current_date.strftime('%Y-%m-%d'))['Close'].iloc[-1]
tesla_current_price = tesla_data.history(end=current_date.strftime('%Y-%m-%d'))['Close'].iloc[-1]
